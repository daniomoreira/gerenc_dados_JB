...
#Lendo os arquivos csv a partir do path
...
##Vamos aplicar as funções head(), dim() e summary() para inspecionar todos os arquivos
...
##Qual a riqueza de cada área? Primeiro, precisamos transformar a nossa matriz que possui dados de abundância em uma matriz de presença e ausência, ou seja, TRUE (>0) e FALSE (<0)
comm.pa <- comm[, -1] > 0
...
##vamos juntar par a par as tabelas usando a função merge()
